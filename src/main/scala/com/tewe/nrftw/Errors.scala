...
object Errors {
  ...
  def errors(fullState: FullState): FullState = {
    ...
    if (ring1Error != fullState.ring1Error || ring2Error != fullState.ring2Error || ring3Error != fullState.ring3Error) {
  ...
  def errors(config: WeaponBuilderConfig, weaponState: WeaponState): WeaponState = {
    ...
    if (rune1Error != weaponState.rune1Error || rune2Error != weaponState.rune2Error || rune3Error != weaponState.rune3Error || rune4Error != weaponState.rune4Error ) {
      println(s"WeaponState errors has changed for ${config.itemConfig.itemSlot}")
